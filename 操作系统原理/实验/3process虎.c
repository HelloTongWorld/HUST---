  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
    ...
    switch( parent->mapped_info[i].seg_type ){
      ...
      case CODE_SEGMENT:
        ...
        user_vm_map(child->pagetable, parent->mapped_info[i].va, parent->mapped_info[i].npages, parent_pa, prot_to_type(PROT_READ | PROT_EXEC, 1));
        sprint("do_fork map code segment at pa:%lx of parent to child at va:%lx.\n", parent_pa, parent->mapped_info[i].va);
      ...
      case DATA_SEGMENT:
        for(int j = 0; j < parent->mapped_info[i].npages; j++ ) {
            ...
            user_vm_map(child->pagetable, parent->mapped_info[i].va+j*PGSIZE, PGSIZE, (uint64)newaddr, prot_to_type(PROT_WRITE | PROT_READ, 1));
        }
        // after mapping, register the vm region (do not delete codes below!)
        child->mapped_info[child->total_mapped_region].va = parent->mapped_info[i].va;
        child->mapped_info[child->total_mapped_region].npages = 
...
int wait(int pid) {
  // 判断是否需要等待任何子进程，或者是等待特定 PID 的子进程
  bool wait_any = (pid == -1);

  // 检查所有进程或特定进程
  for (int i = 0; i < NPROC; i++) {
    if ((wait_any || pid == i) && // 检查是等待任意进程还是特定进程
        procs[i].parent == current && // 检查当前进程是否是该进程的父进程
        procs[i].status == ZOMBIE) { // 检查该进程是否为僵尸进程
      procs[i].status = FREE; // 清理进程
      return i; // 返回进程ID
    }
  }
